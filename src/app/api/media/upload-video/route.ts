...
import { uploadVideoBuffer, isCloudinaryConfigured } from "@/lib/media/cloudinary";
import { authorizeInvitationAccess } from "@/lib/invitation-access";

const MAX_BYTES = 40 * 1024 * 1024; // 40MB
const MAX_VIDEOS_PER_INVITATION = 5;
...
export async function POST(request: Request) {
  ...
  if (file.size > MAX_BYTES) {
    return NextResponse.json({ error: "File is larger than 40MB." }, { status: 400 });
  }
  ...
  const existingCount = await db.media.count({ where: { invitationId, type: "VIDEO" } });
  if (existingCount >= MAX_VIDEOS_PER_INVITATION) {
    return NextResponse.json({ error: "Video limit reached for this invitation." }, { status: 400 });
  }
