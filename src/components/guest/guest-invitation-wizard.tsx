...
import { celebrantNames, DEFAULT_EVENT_CATEGORY } from "@/lib/event-categories";
...
const STEP_FIELDS: Partial<Record<StepKey, (keyof InvitationWizardFormValues)[]>> = {
  culture: ["religion", "caste", "subCaste"],
...
// Which configurable field each wizard input is. Anything absent here is not
// an admin-controlled field (the photo uploader, the creator-gender chooser)
// and is left alone.
const FIELD_KEY_BY_FORM_NAME: Partial<
  Record<keyof InvitationWizardFormValues, InvitationFieldKey>
> = {
  religion: "religion",
  caste: "caste",
  subCaste: "subCaste",
...
export function GuestInvitationWizard({
  ...
  /**
   * The celebration as an admin configured it: which steps to walk, which
   * fields to ask for, and what to call them. Absent — a caller that hasn't
   * been updated — falls back to the built-in catalogue, so the wizard is
   * never left with no form at all.
   */
  categoryConfig?: ResolvedEventCategory;
  themes: Theme[];
  ...
  const category = categoryConfig ?? resolveEventCategory(eventCategory, null);
  const stepKeys = category.steps as StepKey[];
  const rules = category.fields;
  // A field is asked only if its own switch is on *and* the step it lives on
  // is being walked — an admin who removes the Culture step shouldn't have to
  // remember to switch off the three fields that were on it.
  const asks = (key: InvitationFieldKey) => {
    const field = INVITATION_FIELDS.find((f) => f.key === key);
    return Boolean(
      rules[key]?.enabled && field && stepKeys.includes(field.step as StepKey),
    );
  };
  ...
  const visibleThemes = useMemo(
    () =>
      themeCategory === "all"
        ? themes
        : themes.filter((t) => t.category === themeCategory),
  ...
  const relativeFields = useFieldArray({
    ...
    async function ensureDraft() {
      const existing =
        typeof window !== "undefined"
          ? sessionStorage.getItem(DRAFT_ID_STORAGE_KEY)
          : null;
      if (existing) {
  ...
  async function goNext() {
    ...
    // Only validate what this form actually asks for: a field an admin has
    // switched off is never filled in, so validating it would wall people
    // into a step with no visible way past.
    const fields = STEP_FIELDS[currentStep]?.filter(
      (name) => !FIELD_KEY_BY_FORM_NAME[name] || asks(FIELD_KEY_BY_FORM_NAME[name]!),
    );
    ...
    // Required is the admin's call, not the schema's — these fields are all
    // optional on the record, and stay so for invitations made before the
    // rule changed.
    let missing = false;
    for (const [name, key] of Object.entries(FIELD_KEY_BY_FORM_NAME)) {
      const fieldName = name as keyof InvitationWizardFormValues;
      if (!key || !STEP_FIELDS[currentStep]?.includes(fieldName)) continue;
      if (!asks(key) || !rules[key].required) continue;
      ...
      if (empty) {
        form.setError(fieldName, {
          type: "required",
          message: `${fieldLabel(rules, key, INVITATION_FIELDS.find((f) => f.key === key)?.label ?? "This")} is required`,
        });
        missing = true;
      }
    }
  ...
  return (
    <div className="flex flex-col gap-6">
            ...
            <AnimatePresence mode="wait" initial={false}>
              ...
              >
                {currentStep === "culture" && (
                  <>
                    {asks("religion") && (
                      <Field label={fieldLabel(rules, "religion", "Religion")}>
                    ...
                    {asks("caste") && (
                      <Field
                        label={fieldLabel(
                          rules,
                          "caste",
                          `Community or caste${rules.caste.required ? "" : " (optional)"}`,
                        )}
                        error={form.formState.errors.caste?.message}
                      >
                        <Input
                          placeholder="Maratha, Agarwal, Syrian Catholic…"
                          {...form.register("caste")}
                        />
                      </Field>
                    )}
                    {asks("subCaste") && (
                      <Field
                        label={fieldLabel(
                          rules,
                          "subCaste",
                          `Sub-caste${rules.subCaste.required ? "" : " (optional)"}`,
                        )}
                        error={form.formState.errors.subCaste?.message}
                      >
                        <Input
                          placeholder="Deshastha, Chaturth…"
                          {...form.register("subCaste")}
                        />
                      </Field>
                    )}
                  </>
                  ...
                  <>
                    <div className="grid gap-4 sm:grid-cols-2">
                      {(() => {
                        ...
                        if (!asks("secondaryName")) return brideField;
                  ...
                  <div className="flex flex-col gap-5">
                    {(asks("venueName") ||
                      ...
                      <div className="grid gap-4 rounded-lg border p-4">
                        ...
                        {asks("venueName") && (
                          <Field
                            label={fieldLabel(rules, "venueName", "Venue name")}
                        ...
                        {asks("venueAddress") && (
                          <Field
                            label={fieldLabel(rules, "venueAddress", "Venue address")}
                            error={form.formState.errors.venueAddress?.message}
                          >
                        ...
                        {asks("googleMapsUrl") && (
                          <Field
                            label={fieldLabel(
                              rules,
                              "googleMapsUrl",
                              `Google Maps link${rules.googleMapsUrl.required ? "" : " (optional)"}`,
                            )}
                        ...
                        {asks("customMessage") && (
                          <Field
                            label={fieldLabel(
                              rules,
                              "customMessage",
                              `Message for guests${rules.customMessage.required ? "" : " (optional)"}`,
                            )}
                            error={form.formState.errors.customMessage?.message}
                          >
                    ...
                    <div
                      className={cn(
                        "flex-col gap-3",
                        asks("events") ? "flex" : "hidden",
                      )}
                    >
                      <p className="text-sm font-medium">
                        {fieldLabel(rules, "events", category.eventsLabel)}
                      </p>
                      {eventFields.fields.map((field, index) => {
                        ...
                        const extras = [
                          ...
                          },
                          {
                            ...
                            icon: MapPin,
                            label: "Venue",
                            placeholder: "If different from the main venue",
                          },
                          {
                            key: "dressCode",
                            icon: Shirt,
                            label: "Dress code",
                            placeholder: "Traditional, Bridal Gold",
                          },
                          {
                            key: "tagline",
                            icon: Quote,
                            label: "Tagline",
                            placeholder: "The royal procession begins!",
                          },
                        ] as const;
                        ...
                        return (
                          ...
                          >
                            <div className="grid gap-3 sm:grid-cols-[1fr_auto_auto] sm:items-end">
                              <Field label="Name">
                                <Input
                                  {...form.register(`events.${index}.name` as const)}
                                />
                              </Field>
                              <Field label="Date">
                                <Input
                                  ...
                                  .map((extra) => (
                                    <Field key={extra.key} label={extra.label}>
                                      <Input
                                        ...
                                        {...form.register(
                                          `events.${index}.${extra.key}` as const,
                                        )}
                                      />
                                    </Field>
                                  ))}
                  ...
                  <div className="flex flex-col gap-3">
                    {relativeFields.fields.map((field, index) => (
                      ...
                      >
                        <Field label="Name">
                          <Input
                            {...form.register(`familyMembers.${index}.name` as const)}
                          />
                        </Field>
                        <Field label="Relation">
                          <Input
                            ...
                            {...form.register(
                              `familyMembers.${index}.relation` as const,
                            )}
                          />
                        </Field>
                        <Button
                    ...
                    <Button
                      ...
                      variant="outline"
                      onClick={() =>
                        relativeFields.append({ side: "BRIDE", relation: "", name: "" })
                      }
                    >
                      <Plus />
                      Add name
                  ...
                  <>
                    <Field label="Theme">
                      <Tabs
                        value={themeCategory}
                        onValueChange={(v) =>
                          setThemeCategory(v as typeof themeCategory)
                        }
                        className="mb-3"
                      >
                        <TabsList>
                          ...
                          {selectedThemeColorways.map((colorway) => {
                            ...
                            return (
                              <button
                                ...
                                type="button"
                                onClick={() =>
                                  form.setValue("colorwaySlug", colorway.slug)
                                }
                                aria-pressed={active}
                                className={cn(
                  ...
                  <>
                    ...
                    <div className="text-muted-foreground grid gap-1 text-sm">
                      <p className="text-foreground font-medium">
                        {celebrantNames(
                          category,
                          form.watch("brideName") ?? "",
                          form.watch("groomName"),
                        )}
                      </p>
                      <p>
      ...
      <Dialog
        open={genderPromptOpen && isCoupleEvent}
        onOpenChange={setGenderPromptOpen}
      >
        <DialogContent
        ...
        >
          <DialogHeader>
            ...
            <DialogDescription>
              We&apos;ll use this to fill in your name first — you can still edit both
              names.
            </DialogDescription>
          </DialogHeader>
          <div className="mt-2 grid grid-cols-2 gap-3">
            <Button
              ...
              variant="outline"
              size="lg"
              onClick={() => chooseCreatorGender("groom")}
            >
              I&apos;m the Groom
            </Button>
            <Button
              ...
              variant="outline"
              size="lg"
              onClick={() => chooseCreatorGender("bride")}
            >
              I&apos;m the Bride
            </Button>
          </div>
